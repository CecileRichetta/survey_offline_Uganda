...
class C(BaseConstants):
    NAME_IN_URL = 'app_12_LUG'
    PLAYERS_PER_GROUP = None
    NUM_ROUNDS = 1
    TEMPLATE_DEBRIEF_FORM = '_static/texts_LUGANDA/debrief_form.html'
...
class Page1_1(Page):
    ...
    def is_displayed(player):
        participant = player.participant
        return not participant.dropout and player.session.config['name'] == "session_C4P_LUGANDA_w1"
...
class Page1_2(Page):
    ...
    def get_form_fields(player):
        participant = player.participant
        if participant.dropout is False and player.session.config['name'] == "session_C4P_LUGANDA_w2":
            return [
                'recall_phone1'
            ]
        else:
    ...
    def is_displayed(player):
        participant = player.participant
        return not participant.dropout and player.session.config['name'] == "session_C4P_LUGANDA_w2"
...
class Page2(Page):
    ...
    def is_displayed(player):
        ...
        return (not participant.dropout) and (
                ...
                (player.session.config['name'] == "session_C4P_LUGANDA_w2" and participant.treatment_hope == 1)
        )
