...
class Player(BasePlayer):
    ...
    consent = models.BooleanField(choices=[[True, 'Yes'],
                                           [False, 'No']],
                                  label='Itye ki yeyo me bedo ikwan man?',
                                  widget=widgets.RadioSelect)
...
class Page2(Page):
    ...
    def vars_for_template(player):
        ...
        if player.session.config['name'] == "session_C4P_LUO_w1":
            ...
            # Read the text file
            import os
            file_path = os.path.join('_static', 'texts_LUO', text_file)
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        if player.session.config['name']=="session_C4P_LUO_w2":
